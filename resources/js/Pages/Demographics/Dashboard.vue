<template>
  <Head title="Dashboard Statistik Kependudukan" />
  ...
  <AdminLayout>
    <div class="space-y-6">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 class="text-lg sm:text-xl font-black text-gray-900 dark:text-zinc-50 tracking-tight">Dashboard Statistik Kependudukan</h1>
          <p class="text-xs text-gray-400 mt-0.5">
            Data statistik agregat 
            <span v-if="selectedYear2" class="font-bold text-blue-600 dark:text-blue-400">tahun {{ selectedYear2 }} — Semester {{ selectedSem }} ({{ selectedSem == 1 ? 's.d. Juni' : 's.d. Desember' }})</span>
          </p>
        </div>
        <div class="flex flex-wrap items-center gap-2 w-full md:w-auto">
          <select v-model="selectedLevel" @change="onLevelChange" class="px-3 py-2 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl text-xs font-semibold focus:outline-none">
            <option v-for="(label, key) in regionLevels" :key="key" :value="key">{{ label }}</option>
          </select>

          <div v-if="selectedLevel === 'regency'" class="px-3 py-2 bg-gray-100 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 rounded-xl text-xs font-semibold text-gray-500">
            Kabupaten Dompu
          </div>

          <select v-if="selectedLevel === 'district'" v-model="selectedCode" @change="applyFilter" class="px-3 py-2 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl text-xs font-semibold focus:outline-none">
            <option v-for="kec in kecamatans" :key="kec.code" :value="kec.code">{{ kec.name }}</option>
          </select>

          <select v-if="selectedLevel === 'village'" v-model="selectedCode" @change="applyFilter" class="px-3 py-2 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl text-xs font-semibold focus:outline-none">
            <option v-for="desa in desas" :key="desa.code" :value="desa.code">{{ desa.name }}</option>
          </select>

          <select v-model="selectedYear2" @change="applyFilter" class="px-3 py-2 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl text-xs font-semibold focus:outline-none">
            <option v-for="yr in availableYears" :key="yr" :value="yr">Tahun {{ yr }}</option>
          </select>

          <select v-model="selectedSem" @change="applyFilter" class="px-3 py-2 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl text-xs font-semibold focus:outline-none">
            <option v-for="s in availableSemesters" :key="s" :value="s">
              Semester {{ s }} ({{ s == 1 ? 's.d. Juni' : 's.d. Desember' }})
            </option>
          </select>
        </div>
      </div>
      ...
      <template v-else>
      <!-- KPI Summary tiles (Premium Gradient & Glassmorphism Cards) -->
          ...
          <div class="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-3xl p-5 shadow-sm flex flex-col justify-between hover:shadow-md transition">
            ...
            <div class="mt-3 pt-3 border-t border-gray-100 dark:border-zinc-800 text-[10px]">
              <p class="font-bold text-blue-600 dark:text-blue-400">
                ⚡ {{ charts.productive_age?.percentage !== undefined ? charts.productive_age.percentage + '% Bonus Demografi' : (dataAvailable && charts.productive_age?.total ? ((charts.productive_age.total / (charts.population?.total || 1)) * 100).toFixed(1) + '% Bonus Demografi' : 'Data produktif belum ada') }}
              </p>
            </div>
          </div>
          ...
          <div class="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-3xl p-5 shadow-sm flex flex-col justify-between hover:shadow-md transition">
            ...
            <div class="mt-3 pt-3 border-t border-gray-100 dark:border-zinc-800 text-[10px]">
              <p class="font-bold text-amber-600 dark:text-amber-400">
                👴 {{ charts.lansia?.percentage !== undefined ? charts.lansia.percentage + '% Dari Total Penduduk' : (dataAvailable && charts.lansia?.total ? ((charts.lansia.total / (charts.population?.total || 1)) * 100).toFixed(1) + '% Dari Total Penduduk' : 'Data lansia belum ada') }}
              </p>
            </div>
          </div>
      ...
      <!-- Charts section (main grid) -->
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        
        <!-- Population per Kecamatan (Bar Chart) -->
        <div class="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-3xl p-4 sm:p-6 shadow-sm overflow-hidden">
          <h4 class="text-xs font-extrabold text-gray-700 dark:text-zinc-200 mb-3">📍 Penduduk per Kecamatan</h4>
          <div ref="kecamatanChartEl" class="h-64 w-full"></div>
        </div>

        <!-- Population Pyramid -->
        <div class="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-3xl p-4 sm:p-6 shadow-sm overflow-hidden">
          <h4 class="text-xs font-extrabold text-gray-700 dark:text-zinc-200 mb-3">🔻 Piramida Penduduk (Kelompok Usia)</h4>
          <div ref="pyramidChartEl" class="h-64 w-full"></div>
        </div>

        <!-- Religion Donut -->
        <div class="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-3xl p-4 sm:p-6 shadow-sm overflow-hidden">
          <h4 class="text-xs font-extrabold text-gray-700 dark:text-zinc-200 mb-3">☪️ Distribusi Agama</h4>
          <div ref="religionChartEl" class="h-64 w-full"></div>
        </div>

        <!-- Education Horizontal Bar -->
        <div class="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-3xl p-4 sm:p-6 shadow-sm overflow-hidden">
          <h4 class="text-xs font-extrabold text-gray-700 dark:text-zinc-200 mb-3">🎓 Tingkat Pendidikan</h4>
          <div ref="educationChartEl" class="h-64 w-full"></div>
        </div>

        <!-- Marital Status Pie -->
        <div class="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-3xl p-4 sm:p-6 shadow-sm overflow-hidden">
          <h4 class="text-xs font-extrabold text-gray-700 dark:text-zinc-200 mb-3">💍 Status Perkawinan</h4>
          <div ref="maritalChartEl" class="h-64 w-full"></div>
        </div>

        <!-- Blood Type Bar -->
        <div class="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-3xl p-4 sm:p-6 shadow-sm overflow-hidden">
          <h4 class="text-xs font-extrabold text-gray-700 dark:text-zinc-200 mb-3">🩸 Golongan Darah</h4>
          <div ref="bloodChartEl" class="h-64 w-full"></div>
        </div>

        <!-- Disability Chart -->
        <div class="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-3xl p-4 sm:p-6 shadow-sm md:col-span-2 overflow-hidden">
          <h4 class="text-xs font-extrabold text-gray-700 dark:text-zinc-200 mb-3">♿ Ragam Penyandang Disabilitas</h4>
          <div ref="disabilityChartEl" class="h-64 w-full"></div>
        </div>
      </div>

      <!-- Kecamatan population data table -->
      <div class="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-2xl shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-50 dark:border-zinc-800 flex items-center justify-between">
          <div>
            <h4 class="text-xs font-extrabold text-gray-700 dark:text-zinc-200">Ringkasan Data per Kecamatan</h4>
            <p class="text-[9px] text-gray-400 mt-0.5">
              Sumber: dataset <code class="bg-gray-100 dark:bg-zinc-800 px-1 rounded">region_level = district</code> yang telah diupload untuk periode ini.
            </p>
          </div>
          <span v-if="heatmapData && heatmapData.some(k => k.hasData)" class="text-[9px] font-bold text-emerald-600 bg-emerald-50 dark:bg-emerald-900/20 px-2 py-1 rounded-full">{{ heatmapData.filter(k => k.hasData).length }} / {{ heatmapData.length }} kecamatan</span>
          <span v-else class="text-[9px] font-bold text-amber-600 bg-amber-50 dark:bg-amber-900/20 px-2 py-1 rounded-full">Belum ada dataset per kecamatan</span>
        </div>
        <div class="overflow-x-auto">
          <table class="w-full text-xs">
            <thead class="bg-gray-50 dark:bg-zinc-800/60">
              <tr>
                <th class="text-left px-3 py-2 sm:px-5 sm:py-2.5 text-[8px] sm:text-[9px] font-bold uppercase tracking-wider text-gray-400 whitespace-nowrap">Kode Wilayah</th>
                <th class="text-left px-3 py-2 sm:px-5 sm:py-2.5 text-[8px] sm:text-[9px] font-bold uppercase tracking-wider text-gray-400 whitespace-nowrap">Kecamatan</th>
                <th class="text-right px-3 py-2 sm:px-5 sm:py-2.5 text-[8px] sm:text-[9px] font-bold uppercase tracking-wider text-gray-400 whitespace-nowrap">Penduduk</th>
                <th class="text-right px-3 py-2 sm:px-5 sm:py-2.5 text-[8px] sm:text-[9px] font-bold uppercase tracking-wider text-gray-400 whitespace-nowrap">Laki-laki</th>
                <th class="text-right px-3 py-2 sm:px-5 sm:py-2.5 text-[8px] sm:text-[9px] font-bold uppercase tracking-wider text-gray-400 whitespace-nowrap">Perempuan</th>
                <th class="text-right px-3 py-2 sm:px-5 sm:py-2.5 text-[8px] sm:text-[9px] font-bold uppercase tracking-wider text-gray-400 whitespace-nowrap">Luas km²</th>
                <th class="text-right px-3 py-2 sm:px-5 sm:py-2.5 text-[8px] sm:text-[9px] font-bold uppercase tracking-wider text-gray-400 whitespace-nowrap">Kepadatan</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-50 dark:divide-zinc-800/60">
              <tr v-for="kec in sortedHeatmapData" :key="kec.id" class="hover:bg-gray-50/50 dark:hover:bg-zinc-800/30 transition">
                <td class="px-3 py-2 sm:px-5 sm:py-3 font-mono font-extrabold text-blue-600 dark:text-blue-400 text-[10px] sm:text-[11px] whitespace-nowrap">{{ kec.code }}</td>
                <td class="px-3 py-2 sm:px-5 sm:py-3 font-bold text-gray-700 dark:text-zinc-200 text-xs whitespace-nowrap">{{ kec.name }}</td>
                <td class="px-3 py-2 sm:px-5 sm:py-3 text-right font-mono font-semibold text-xs whitespace-nowrap">
                  <span v-if="kec.hasData !== false" class="text-gray-600 dark:text-zinc-400">{{ formatNumber(kec.population ?? kec.population_total) }}</span>
                  <span v-else class="text-gray-300 dark:text-zinc-600 italic text-[10px]">Belum ada data</span>
                </td>
                <td class="px-3 py-2 sm:px-5 sm:py-3 text-right font-mono text-xs whitespace-nowrap">
                  <span v-if="kec.hasData !== false" class="text-sky-600">{{ formatNumber(kec.male ?? kec.male_count) }}</span>
                  <span v-else class="text-gray-300 dark:text-zinc-600">—</span>
                </td>
                <td class="px-3 py-2 sm:px-5 sm:py-3 text-right font-mono text-xs whitespace-nowrap">
                  <span v-if="kec.hasData !== false" class="text-pink-600">{{ formatNumber(kec.female ?? kec.female_count) }}</span>
                  <span v-else class="text-gray-300 dark:text-zinc-600">—</span>
                </td>
                <td class="px-3 py-2 sm:px-5 sm:py-3 text-right font-mono text-gray-400 text-xs whitespace-nowrap">{{ kec.area_km2 ? Number(kec.area_km2).toFixed(2) : '—' }}</td>
                <td class="px-3 py-2 sm:px-5 sm:py-3 text-right font-mono text-gray-400 text-xs whitespace-nowrap">
                  <span v-if="kec.hasData !== false && (kec.population ?? kec.population_total) && kec.area_km2">
                    {{ Math.round((kec.population ?? kec.population_total) / kec.area_km2) }} /km²
                  </span>
                  <span v-else class="text-gray-300 dark:text-zinc-600">—</span>
                </td>
              </tr>
...
const selectedYear2 = ref(props.selectedYear ?? props.latestYear);
const selectedSem   = ref(props.selectedSemester ?? 1);

watch(() => props.latestYear, (newYear) => {
  if (newYear) selectedYear2.value = newYear;
...
watch(() => props.selectedSemester, (newSem) => {
  if (newSem) selectedSem.value = newSem;
});

watch(() => props.selectedRegionLevel, (val) => {
  if (val) selectedLevel.value = val;
});

watch(() => props.selectedRegionCode, (val) => {
  if (val) selectedCode.value = val;
});
...
const onLevelChange = () => {
  ...
  } else if (selectedLevel.value === 'district') {
    selectedCode.value = props.kecamatans?.[0]?.code || '';
  } else if (selectedLevel.value === 'village') {
    selectedCode.value = props.desas?.[0]?.code || '';
  }
  applyFilter();
};

const applyFilter = () => {
  router.get(route('admin.demographics.dashboard'), {
    region_level: selectedLevel.value,
    region_code: selectedCode.value,
    year: selectedYear2.value,
...
const initKecamatanChart = () => {
  if (!kecamatanChartEl.value) return;
  const chart = echarts.init(kecamatanChartEl.value);
  ...
  if (!hasAnyData) {
    chart.setOption({
      title: { text: 'Belum ada dataset per kecamatan', left: 'center', top: 'middle', textStyle: { color: '#9ca3af', fontSize: 11 } }
    });
    return;
  }
...
const initPyramidChart = () => {
  if (!pyramidChartEl.value) return;
  const chart  = echarts.init(pyramidChartEl.value);
  chartInstances.push(chart);
  chart.clear();
  const data   = props.charts?.population;

  if (!data) {
    chart.setOption({ title: { text: 'Tidak ada data piramida penduduk', left: 'center', top: 'middle', textStyle: { color: '#9ca3af', fontSize: 11 } } });
    return;
  }

  const categories = data.categories || [];
  const male       = (data.male || []).map(v => -Math.abs(v));
  const female     = data.female || [];

  chart.setOption({
    tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' }, formatter: (params) => {
      const cat = categories[params[0].dataIndex];
      return `${cat}<br/>Laki-laki: ${Math.abs(params[0]?.value || 0).toLocaleString('id-ID')}<br/>Perempuan: ${(params[1]?.value || 0).toLocaleString('id-ID')}`;
    }},
    ...
    yAxis: { type: 'category', data: categories, axisLabel: { fontSize: 8 } },
    series: [
      { name: 'Laki-laki', type: 'bar', stack: 'total', data: male, itemStyle: { color: '#06b6d4' }, barMaxWidth: 14 },
      { name: 'Perempuan', type: 'bar', stack: 'total', data: female, itemStyle: { color: '#ec4899' }, barMaxWidth: 14 },
    ],
  });
};

const initReligionChart = () => {
  if (!religionChartEl.value) return;
  const chart = echarts.init(religionChartEl.value);
  chartInstances.push(chart);
  chart.clear();
  const data  = props.charts?.religion;

  if (!data || !data.items) {
    chart.setOption({ title: { text: 'Tidak ada data agama', left: 'center', top: 'middle', textStyle: { color: '#9ca3af', fontSize: 11 } } });
    return;
  }
  ...
  chart.setOption({
    ...
    series: [{
      ...
      data: data.items.map((item, i) => ({ name: item.name, value: item.value, itemStyle: { color: COLORS[i % COLORS.length] } })),
      label: { fontSize: 8 },
    }],
  });
};
...
const initEducationChart = () => {
  if (!educationChartEl.value) return;
  const chart = echarts.init(educationChartEl.value);
  chartInstances.push(chart);
  chart.clear();
  const data  = props.charts?.education;

  if (!data || !data.categories) {
    chart.setOption({ title: { text: 'Tidak ada data pendidikan', left: 'center', top: 'middle', textStyle: { color: '#9ca3af', fontSize: 11 } } });
    return;
  }

  chart.setOption({
    tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } },
    grid: { left: 5, right: 20, top: 10, bottom: 10, containLabel: true },
    xAxis: { type: 'value', axisLabel: { fontSize: 8, formatter: v => (v >= 1000 ? (v/1000).toFixed(0)+'K' : v) } },
    yAxis: { type: 'category', data: data.categories, axisLabel: { fontSize: 8 } },
    series: [{ type: 'bar', data: data.values, itemStyle: { color: '#10b981' }, barMaxWidth: 16 }],
  });
};

const initMaritalChart = () => {
  if (!maritalChartEl.value) return;
  const chart = echarts.init(maritalChartEl.value);
  chartInstances.push(chart);
  chart.clear();
  const data  = props.charts?.marital;

  if (!data || !data.items) {
    chart.setOption({ title: { text: 'Tidak ada data perkawinan', left: 'center', top: 'middle', textStyle: { color: '#9ca3af', fontSize: 11 } } });
    return;
  }
...
const initBloodChart = () => {
  if (!bloodChartEl.value) return;
  const chart = echarts.init(bloodChartEl.value);
  chartInstances.push(chart);
  chart.clear();
  const data  = props.charts?.blood_type;

  if (!data || !data.categories) {
    chart.setOption({ title: { text: 'Tidak ada data golongan darah', left: 'center', top: 'middle', textStyle: { color: '#9ca3af', fontSize: 11 } } });
    return;
  }
  ...
  chart.setOption({
    ...
    xAxis: { type: 'category', data: data.categories, axisLabel: { fontSize: 10, fontWeight: 'bold' } },
    yAxis: { type: 'value', axisLabel: { fontSize: 8, formatter: v => (v >= 1000 ? (v/1000).toFixed(0)+'K' : v) } },
    series: [{
      type: 'bar', data: data.values,
      itemStyle: { color: (params) => COLORS[params.dataIndex % COLORS.length] },
      barMaxWidth: 32,
      label: { show: true, position: 'top', fontSize: 8, formatter: (params) => (params.value || 0).toLocaleString('id-ID') },
    }],
  });
};

const initDisabilityChart = () => {
  if (!disabilityChartEl.value) return;
  const chart = echarts.init(disabilityChartEl.value);
  chartInstances.push(chart);
  chart.clear();
  const data  = props.charts?.disability;

  if (!data || !data.items) {
    chart.setOption({ title: { text: 'Tidak ada data ragam disabilitas', left: 'center', top: 'middle', textStyle: { color: '#9ca3af', fontSize: 11 } } });
    return;
  }
  ...
  chart.setOption({
    ...
    yAxis: { type: 'category', data: data.items.map(i => i.name), axisLabel: { fontSize: 8 } },
    series: [{
      type: 'bar', data: data.items.map(i => i.value),
